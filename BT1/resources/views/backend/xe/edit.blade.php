...
@section('content')
    <div class="container" style="margin-left:280px">
        <h1 class="text-center">SỬA THÔNG TIN XE</h1>
        @if ($errors->any())
        ...
        <form action="{{ route('cars.update',['car' => $car->id]) }}" method="post" id="add_car">
            @csrf
            @method('PUT')
            <div class="row">
                ...
                <div class="form-group col-md-2">
                    ...
                    <select name="typecar" id="typecar" class="form-control">
                        ...
                        @foreach ($type_car as $item)
                            <option {{ $car->type_car_id == $item->id ? 'selected' : '' }} value="{{ $item->id }}">
                                {{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col-md-2">
                    ...
                    <select name="labelcar" id="labelcar" class="form-control">
                        ...
                        @foreach ($label_car as $item)
                            <option {{ $car->label_car_id == $item->id ? 'selected' : '' }} value="{{ $item->id }}">
                                {{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col-md-2">
                    ...
                    <select name="number_type_car" id="number_type_car" class="form-control">
                        ...
                        @foreach ($number_type_car as $item)
                            <option {{ $car->number_type_car_id == $item->id ? 'selected' : '' }}
                ...
                <div class="form-group col-md-2">
                    ...
                    <select name="dvvc" id="dvvc" class="form-control">
                        @foreach ($dvvc as $item)
                            <option {{ $car->dvvc_id == $item->id ? 'selected' : '' }} value="{{ $item->id }}">
                                {{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col-md-2">
                    ...
                    <select name="romooc" id="romooc" class="form-control">
                        ...
                        @foreach ($romooc as $item)
                            <option {{ $car->romooc_id == $item->id ? 'selected' : '' }} value="{{ $item->id }}">
                                {{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col-md-2">
                    ...
                    <select name="driver" id="driver" class="form-control">
                        ...
                        @foreach ($driver as $item)
                            <option {{ $car->driver_id == $item->id ? 'selected' : '' }} value="{{ $item->id }}">
                                {{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col-md-2">
                    ...
                    <select name="subdriver" id="subdriver" class="form-control">
                        ...
                        @foreach ($sub_driver as $item)
                            <option {{ $car->sub_driver_id == $item->id ? 'selected' : '' }} value="{{ $item->id }}">
                                {{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col-md-2">
                    ...
                    <select name="type_list" id="type_list" class="form-control">
                        ...
                        @foreach ($type_list as $item)
                            <option {{ $car->type_list_id == $item->id ? 'selected' : '' }} value="{{ $item->id }}">
                                {{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col-md-2">
                    ...
                    <select name="car_pump" id="car_pump" class="form-control">
                        ...
                        <option value="có" {{ $car->car_pump == 'có' ? 'selected' : '' }}>có</option>
                        <option value="không" {{ $car->car_pump == 'không' ? 'selected' : '' }}>không</option>
                    </select>
                </div>
                <div class="form-group col-md-2">
                    ...
                    <select name="wheel_structure" id="wheel_structure" class="form-control">
                        ...
                        @foreach ($wheel_structure as $item)
                            <option {{ $car->wheel_structure_id == $item->id ? 'selected' : '' }}
                                value="{{ $item->id }}">{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col-md-2">
                    ...
                    <select name="fuel" id="fuel" class="form-control">
                        ...
                        @foreach ($fuel as $item)
                            <option {{ $car->fuel_id == $item->id ? 'selected' : '' }} value="{{ $item->id }}">
                                {{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col-md-2">
                    ...
                    <input class="form-control" type="text" name="year_of_manufacture" id="year_of_manufacture"
                        placeholder="Nhập năm sản xuất" value="{{ $car->year_of_manufacture }}">
                </div>
                <div class="form-group col-md-2">
                    <label>Định mức NL(lít/100km)</label><span class="text-danger" id="error_fuel_norms"></span>
                    <input class="form-control" type="text" name="fuel_norms" id="fuel_norms"
                        value="{{ $car->fuel_norms }}">
                </div>
                <div class="form-group col-md-2">
                ...
                <div class="form-group col-md-2">
                    ...
                    <select name="status" id="status" class="form-control">
                        <option {{ $car->status == '1' ? 'selected' : '' }} value="1">Đang sử dụng</option>
                        <option {{ $car->status == '0' ? 'selected' : '' }} value="0">Ngưng sử dụng</option>
                    </select>
                </div>
                <div class="form-group col-md-2">
                    <label>Ngày ngừng sử dụng</label><span class="text-danger" id="error_end_date"></span>
                    <input class="form-control" type="date" name="end_date" id="end_date"
                        value="{{ $car->end_date }}" {{$car->status == 1 ? 'disabled' : ''}} >
                </div>
                <div class="form-group col-md-2">
                ...
                <div class="form-group col-md-4">
                    ...
                    <textarea name="info_car" id="info_car" cols="30" rows="5" class="form-control"
                        placeholder="Thông tin xe">{{ $car->info_car }}</textarea>
                </div>
                <div class="form-group col-md-4">
                    <label>Ghi chú</label>
                    <textarea name="note" id="note" cols="30" rows="5" class="form-control"
                        placeholder="Nhập ghi chú">{{ $car->note }}</textarea>
                </div>
                <div class="form-group col-md-4">
                    <label>Ghi chú thống kê tồn</label>
                    <textarea name="inventory" id="inventory" cols="30" rows="5" class="form-control"
                        placeholder="Nhập ghi chú thống kê tồn">{{ $car->inventory }}</textarea>
                </div>
                <div class="mb-5">
                    <input type="hidden" name="id_car" id="id_car" value="{{ $car->id }}">
                    <a class="btn btn-warning" href="{{ route('cars.index') }}" role="button">Quay lại</a>
                    <button type="submit" class="btn btn-success" id="save_button">Cập nhật</button>
                </div>
            </div>
        </form>
    </div>
...
<script>
    $(document).ready(function() {
        ...
        // Chức năng sửa xe dùng của cái lưu
        $("#add_car").submit(function(e) {
            e.preventDefault();
            var id = $("#id_car").val();
            $.ajax({
                type: "PUT",
                url: "{{ route('cars.update', '') }}" + '/' + id,
                data: $(this).serialize(),
